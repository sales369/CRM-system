...
# ── 3. ENTERPRISE UI & CSS ARCHITECTURE ────────────────────────────────────────
st.markdown("""
<style>
...
/* Force Streamlit UI icons to use Material font */
...
/* ── TYPOGRAPHY ── */
.page-title { font-size: 2.5rem; font-weight: 800; color: #0f172a; margin: 0 0 4px; letter-spacing: -0.04em; }
...
/* ── GLASS FORMS & CONTAINERS ── */
.form-section {
    background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px);
    border-radius: 20px; padding: 32px 36px; border: 1px solid rgba(255,255,255,0.9);
    box-shadow: 0 10px 40px rgba(15, 23, 42, 0.03); margin-bottom: 24px; transition: all 0.3s ease;
}
.form-section:hover { background: rgba(255, 255, 255, 0.8); box-shadow: 0 15px 45px rgba(15, 23, 42, 0.05); }
...
[data-baseweb="input"] > div, [data-baseweb="select"] > div, [data-baseweb="textarea"] > div {
    ...
    border: 1px solid rgba(15, 23, 42, 0.08) !important; transition: all 0.3s ease !important;
}
...
/* ── DYNAMIC LIST STRIPS ── */
.strip { 
    background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(16px); border-radius: 18px; padding: 20px 28px; margin-bottom: 16px; 
    border: 1px solid rgba(255,255,255,1); border-left-width: 6px; display: flex; flex-direction: column; gap: 8px; transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1);
...
def page_dashboard():
    ...
    # Reduced to 3 columns since we removed the 7-day forecast tile
    c1, c2, c3 = st.columns(3)

    # Empty string "\u200B" acts as an invisible label for the button. The CSS handles the rest.
    with c1:
...
def page_user_management():
        ...
        if users.empty: st.info("No operators found.")
            ...
            for _, u in users.iterrows():
                ...
                with c2:
                    ...
                    else: st.markdown("<div style='text-align: center; margin-top: 10px; font-weight: 700; color: #10b981; font-size: 0.85rem;'>Current Operator</div>", unsafe_allow_html=True)
                        
                with c3:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if u["username"] != st.session_state.get("username"):
                        if st.button("Erase Node", key=f"del_{u['id']}", use_container_width=True): db.delete_user(int(u["id"])); st.rerun()
    ...
    with tab2:
        ...
        if add_submitted:
            if not new_fullname.strip() or not new_username.strip() or not new_password: st.error("❌ Missing required fields.")
            elif len(new_password) < 6: st.error("❌ Access Key too weak (minimum 6 characters).")
            elif db.username_exists(new_username): st.error("❌ Handle already active.")
            else:
