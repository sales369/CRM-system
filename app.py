        ...
        with t1:
            ...
            with sc3:
                ...
                if st.button("✅ Update", type="primary", use_container_width=True, key=f"btn_resched_{cid}"):
                    try:
                        parsed_time = datetime.strptime(new_t_str.strip(), "%H:%M").time()
...
def page_add_client():
    ...
    with st.form("add_client_form", clear_on_submit=True):
        ...
        with c4:
            try:
                ...
                date_str   = str(next_d.strftime("%A, %b %d"))
                time_str   = "⚠️ Invalid time"
                time_valid = False
    ...
    if submitted:
        if not name.strip():
            ...
            st.error("❌ Please enter a valid time in HH:MM format (e.g. 14:30 or 09:00).")
        else:
            parsed_t    = datetime.strptime(next_t_str.strip(), "%H:%M").time()
